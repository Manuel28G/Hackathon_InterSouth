...
func main() {
	...
	data, err := ioutil.ReadFile("File/Entrada.txt") // just pass the file name
	if err != nil {
	...
	//data := `,2,,,48,,,54,,,6,,9,,,,,,,1,,,5,7,68,,,,3,1,,,8,,,2,,,3,,,,,,,,,7,9,8,52,,,,7,,,,`;
	...
	for i, row := range solution {
		for j, cell := range row {
			if (cell.solved) {
				...
				textoSalida +=  " " + strconv.Itoa(cell.Solution) + " "
			} else {
				fmt.Print(" , ")
				textoSalida += " , "
			}
	...
	file, err := os.Create("File/Salida.txt")
	if err != nil {
		fmt.Print("Error al crear archivo")
		return
	}

	file.WriteString(string(textoSalida))
	fmt.Println("Duracion: " + TiempoF.Sub(TiempoI).String())
	file.WriteString("Duracion: " + TiempoF.Sub(TiempoI).String())
	defer file.Close()
}
